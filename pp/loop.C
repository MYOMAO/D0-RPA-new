...
int loop(TString infile="", TString outfile="", Bool_t REAL=true, Bool_t isPbPb=false, Int_t startEntries=0, Int_t endEntries=-1, Bool_t skim=true, Bool_t gskim=true, Bool_t checkMatching=true, Bool_t iseos=false, Bool_t SkimHLTtree=false)
{
  if(istest)
    {
      infile="/mnt/T2_US_MIT/hadoop/cms/store/user/zzshi/Data/MinimumBias1/crab_Dfinder82/170530_225223/0000/finder_pp_12.root";
      outfile="test1.root";
      REAL=true;
      isPbPb=false;
      skim=true;
 ...
 if(i%1000==0) cout<<setw(7)<<i<<" / "<<endEntries<<endl;
      ...
      if(!REAL) fillZeroCandEvt = false;// on MC one should not ignore any reco event 
